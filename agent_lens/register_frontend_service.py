...
ZARR_DATASET_PATH = "/mnt/shared_documents/offline_stitch_20251215-fullplate_2025-12-16_16-50-51.505093/data.zarr"
...
async def setup_service(server, server_id="agent-lens"):
    ...
    def resize_and_pad_to_square_rgb(cell_rgb_u8: np.ndarray, out_size: int, pad_value: int) -> np.ndarray:
        """
        Resize and pad cell image to a square for consistent CLIP embedding.
        
        This ensures all cells have the same input size (224x224 for CLIP ViT-B/32),
        preventing CLIP's preprocessor from doing variable cropping that can change
        the embedding based on the original crop's aspect ratio.
        
        Args:
            cell_rgb_u8: (H,W,3) uint8 RGB image
            out_size: Target square size (e.g., 224 for CLIP)
    ...
    def _process_single_cell(
        ...
        try:
            ...
            # Resize and pad to 224x224 square for consistent CLIP embedding
            # This prevents CLIP's preprocessor from doing variable cropping
