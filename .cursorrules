...
### 2.4. Tools Service Architecture (AI/ML Operations)

The `register_tools_service.py` module provides AI/ML processing capabilities separate from the frontend service for better code organization and maintainability.

#### Tools Service Registration Pattern
```python
async def setup_service(server, server_id="agent-lens-tools"):
    # Register generic RPC service with Hypha-RPC
    await server.register_service({
        "id": server_id,
        "name": "Agent Lens Tools",
        "type": "generic",  # Pure RPC service (not ASGI)
        "config": {"visibility": "public"},
...
- `generate_text_embedding()` - CLIP text embeddings for similarity search
- `generate_image_embeddings_batch()` - Batch CLIP/DINOv2 image embeddings
- Model preloading for faster startup

**Cell Segmentation & Analysis:**
- `build_cell_records()` - Extract cell metadata, crops, and embeddings
- Background worker queues for async processing

**Microscope Integration:**
- `snap_segment_extract_put_queue()` - Server-side image acquisition + segmentation
- `poll_snap_segment_extract_status()` - Poll status for snap+segment 
...
The tools service uses async queues for non-blocking image processing:

```python
# Queue infrastructure
segment_queue = asyncio.Queue()  # Segmentation jobs
build_queue = asyncio.Queue()    # Cell record building
snap_queue = asyncio.Queue()     # Microscope snap jobs

# Background workers
asyncio.create_task(_segment_worker())  # Processes segmentation
asyncio.create_task(_build_worker())    # Builds cell records
asyncio.create_task(_snap_segment_extract_worker())  # Snap + segment
```

**Worker Flow:**
1. Client enqueues job via RPC method
2. Background worker processes job asynchronously
3. Results accumulated in shared list
4. Client waits for completion and retrieves results

#### Health Checks for Tools Service

Separate health probes monitor AI/ML service health:

```python
# Health check endpoints
- Liveness: {SERVER_URL}/{workspace}/services/agent-lens-tools/probes/liveness
- Readiness: {SERVER_URL}/{workspace}/services/agent-lens-tools/probes/readiness

# Checks performed:
- ChromaDB connection (cell storage)
- Segmentation service connection
- Background queue status
```

#### Service Separation Benefits

1. **Separation of Concerns**: Frontend serving vs AI/ML processing
2. **Independent Scaling**: Can scale tools service separately if needed
3. **Better Maintainability**: Smaller, focused files (~1500 lines each)
4. **Independent Health Monitoring**: Monitor frontend and tools separately
5. **Same Process Deployment**: Both services run in same container for simplicity

### 2.5. Data Storage & Image Processing
...
├── register_frontend_service.py  # Frontend ASGI service registration (~1500 lines)
├── register_tools_service.py     # AI/ML tools service registration (~1500 lines)
...
│   │   ├── SimilaritySearchPanel.css  # Similarity search panel styles
│   │   ├── SimilaritySearchPanel.jsx  # Main similarity search control panel
│   │   ├── SimilarityResultInfoWindow.jsx  # Similarity result info window
│   │   ├── SimilarityResultsRenderer.jsx  # Similar annotation rendering
│   │   └── SimilarAnnotationRenderer.jsx  # Similar annotation rendering (legacy)
...
- **Dependencies**: CLIP, torch for vector similarity tests, Playwright for browser testing
...
# Slow tests with AI models (CLIP/DINOv2 vector similarity)
